...
def call_claude(prompt: str, timeout: int = 300, image_paths: list = None) -> dict:
    ...
    # 构建命令：prompt 走 stdin（避免 Windows 命令行参数 8191 字符限制）
    cmd = [claude]
    ...
    try:
        p = subprocess.Popen(
            cmd,
            stdin=subprocess.PIPE,           # prompt 从 stdin 传入，不受命令行长度限制
...
def call_claude_stream(prompt: str, timeout: int = 300, image_paths: list = None,
                       on_text=None, on_done=None) -> None:
    """
    流式调用 Claude Code，通过回调实时返回 token。

    on_text(str)  — 每收到一段 delta 文本就调用
    on_done(dict) — 流结束后调用，参数: {"ok": bool, "elapsed": float, "error": str|None}

    底层使用 Claude Code CLI 的 stream-json 输出格式：
      claude --output-format stream-json --include-partial-messages --print
    """
    claude = find_claude()
    if not claude:
        if on_done:
            on_done({"ok": False, "error": "找不到 claude 命令，请确认 Claude Code 已安装", "elapsed": 0})
        return

    log(f"流式调用 Claude: {prompt[:100]}...")
    if image_paths:
        log(f"  附带 {len(image_paths)} 张图片")
    ...
    cmd = [claude, "--output-format", "stream-json", "--include-partial-messages",
           "--verbose", "--print", "--dangerously-skip-permissions"]
    for ip in (image_paths or []):
        cmd.extend(["--image", ip])
    ...
    try:
        ...
        for line in p.stdout:
            ...
            if inner.get("type") == "content_block_start":
                ...
                _in_thinking = (block.get("type") == "thinking")
                continue
            elif inner.get("type") == "content_block_stop":
                _in_thinking = False
                continue

            # 只处理 text delta，跳过 thinking
            if _in_thinking:
                continue
        ...
        log(f"流式完成 ({elapsed:.1f}s, rc={p.returncode})")

        if on_done:
            ok = p.returncode == 0
            on_done({"ok": ok, "elapsed": round(elapsed, 1), "error": err if not ok else None})
        ...
        elapsed = _time.time() - t0
        if on_done:
            on_done({"ok": False, "error": f"Claude 超时 ({timeout}s)", "elapsed": round(elapsed, 1)})
    except Exception as e:
        elapsed = _time.time() - t0
        log(f"流式异常: {e}")
        if on_done:
            on_done({"ok": False, "error": str(e), "elapsed": round(elapsed, 1)})
